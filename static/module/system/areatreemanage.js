...
define(function (require, exports, module) {
    ...
    getMaxId();
    function getMaxId() {
        api.system.areaManage.getAreaMaxId(function (rep) {
            if (rep.result > 0) {
    ...
    function zTreeOnRename(event, treeId, treeNode, isCancel) {
        api.system.areaManage.updateArea(treeNode.id, treeNode.name, function (rep) {
    ...
    function addTreeNode() {
        ...
        if (zTree.getSelectedNodes()[0]) {
            newNode = {name: "新增标签", area_parent: zTree.getSelectedNodes()[0].id, id: maxId};
            console.log(zTree.getSelectedNodes()[0]);
            zTree.addNodes(zTree.getSelectedNodes()[0], newNode);

        } else {
            newNode = {name: "新增标签", area_parent: 0, id: maxId};
            zTree.addNodes(null, newNode);
        }
        api.system.areaManage.insertArea(JSON.stringify(newNode), function (rep) {
            if (rep.result > 0) {
                getMaxId();
                layer.msg(' 添 加 成 功 ！', {
                    icon: 1,
    ...
    function removeTreeNode() {
        ...
        api.system.areaManage.deleteArea(idS.join(','), function (rep) {
